...
export class TotalConsultationReportComponent implements OnInit {
  ...
  searchReport() {
    ...
    this.schedulerService.getTotalConsultationReports(reqObjForTotalConsultationReport).subscribe({next:(response: any) => {
  ...
  checkDataForNull() {
    let array = this.totalConsultationList.filter(function (obj: any) {
      for (let key in obj) {
        if (obj[key] == null) {
          obj[key] = "";
        }
