...
export class ConsultationReportComponent implements OnInit {
  ...
  searchReport(fromDate: any, toDate: any) {
    ...
    this.schedulerService.getConsultantReport(reqObjForConsultantReport).subscribe({next:(response: any) => {
  ...
  checkDataForNull() {
    let array = this.consultationReportList.filter(function (obj: any) {
      for (let key in obj) {
        if (obj[key] === null) {
          obj[key] = "";
        }
