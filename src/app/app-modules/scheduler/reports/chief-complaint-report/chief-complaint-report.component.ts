...
export class ChiefComplaintReportComponent implements OnInit {
  ...
  searchReport() {
    ...
    this.schedulerService.getChiefComplaintReports(reqObjForChiefCompalintReport).subscribe({next:(response: any) => {
    ...
    if (this.chiefComplaintRawData.length > 0) {
      ...
      for (const element of this.chiefComplaintRawData) {
        if (element.vanID) {
          ...
          let head = Object.keys(headKey);
          console.log('headKey', headKey);

          const report_worksheet: XLSX.WorkSheet = XLSX.utils.json_to_sheet(element.chiefComplaintReport, { header: (head) });
  ...
  modifyHeader(headers: any, i: any) {
    ...
    modifiedHeader = modifiedHeader.charAt(0).toUpperCase() + modifiedHeader.slice(1);
